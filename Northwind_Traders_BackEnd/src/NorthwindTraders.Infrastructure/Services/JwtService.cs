...
public class JwtService
{
    ...
    public string GenerateToken(Employee employee)
    {
        ...
        {
            throw new InvalidOperationException($"Invalid JWT configuration value (e.g. ExpiryMinutes): {ex.Message}", ex);
        }
        catch (ArgumentException ex)
        {
            throw new InvalidOperationException($"Invalid argument while building the JWT token: {ex.Message}", ex);
        }
        catch (Microsoft.IdentityModel.Tokens.SecurityTokenException ex)
        {
            throw new InvalidOperationException($"JWT signing failed: {ex.Message}", ex);
        }
        catch (Exception ex)
        {
            throw new InvalidOperationException($"Failed to generate authentication token: {ex.Message}", ex);
        }
    }
}